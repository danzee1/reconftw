...
ui_init() {
    ...
    # Check for UTF-8 support (important for Ubuntu minimal images on VPS)
    ...
    # Icons & Spinners
    if [[ "$_UI_NO_COLOR" == true ]] || [[ "$locale_utf8" == false ]]; then
        icon_ok="OK" icon_warn="WARN" icon_fail="FAIL" icon_skip="SKIP" icon_cache="CACHE" icon_run="RUN" icon_info="INFO"
        _UI_SPINNER_CHARS=("-" "\\" "|" "/")
    else
        icon_ok="✔" icon_warn="⚠" icon_fail="✖" icon_skip="⏭" icon_cache="♻" icon_run="▹" icon_info="ℹ"
        _UI_SPINNER_CHARS=("⠋" "⠙" "⠹" "⠸" "⠼" "⠴" "⠦" "⠧" "⠇" "⠏")
    fi
    ...
    if [[ "$PREMIUM_UI" == "true" ]] && [[ "$_UI_NO_COLOR" != "true" ]]; then
        # Premium color palette - softer, more professional look
        # Cyan for info (primary accent)
        pcyan="\e[1;38;2;86;182;194m"
        # Soft green for success
        pgreen="\e[1;38;2;152;195;121m"
        # Amber for warnings
        pam="\e[1;38;2;229;192;123m"
        # Rose for errors/critical
        prose="\e[1;38;2;240;113;120m"
        # Various grays for text hierarchy
        g0="\e[38;2;248;249;250m"   # near-white
        g1="\e[38;2;233;236;239m" # light gray
        g2="\e[38;2;134;142;150m" # medium gray
        g3="\e[38;2;73;80;87m"    # dark gray
        # White
        white="\e[1;97m"
        # Reset
        reset="\e[0m"
        # Apply premium colors to standard variables
        bblue="${pcyan}"
        bgreen="${pgreen}"
        byellow="${pam}"
        bred="${prose}"
        ...
        cyan="${pcyan}"
        # Set premium UI flag
        _UI_PREMIUM_MODE=true
    else
        _UI_PREMIUM_MODE=false
    fi

    # Responsive layout: detect compact mode for narrow terminals
    if [[ -z "${_UI_COMPACT_MODE:-}" ]]; then
        local term_width
        term_width=$(ui_term_width 2>/dev/null || echo 80)
        if (( term_width < 60 )); then
            _UI_COMPACT_MODE=true
        else
            _UI_COMPACT_MODE=false
        fi
    fi
...
# Get a summary of vulnerabilities for the live HUD
# Usage: ui_get_vulns_summary [always_show]
# If always_show is true, returns HUD even when no vulns found (with zero counts)
ui_get_vulns_summary() {
    local always_show="${1:-false}"
    local now
    now=$(date +%s)
    if (( now - _UI_HUD_CACHE_TIME < 3 )) && [[ -n "$_UI_HUD_CACHE_VAL" ]]; then
        printf "%s" "$_UI_HUD_CACHE_VAL"
        return 0
    fi
    local -A counts=( [critical]=0 [high]=0 [medium]=0 [low]=0 [info]=0 )
    local out_dir="${OUTPUT_DIR:-${dir:-.}}"
    local sev found=false
    if [[ -d "${out_dir}/nuclei_output" ]]; then
        for sev in "${!counts[@]}"; do
            if [[ -s "${out_dir}/nuclei_output/${sev}.txt" ]]; then
                counts[$sev]=$(sed '/^$/d' "${out_dir}/nuclei_output/${sev}.txt" | wc -l | tr -d ' ')
                found=true
            fi
        done
    fi

    # Build HUD with severity badges
    local hud=""
    local has_findings=false

    # Critical - always show if > 0, red badge
    if [[ ${counts[critical]} -gt 0 ]]; then
        hud+="${bred:-}◆${counts[critical]}${reset:-} "
        has_findings=true
    fi

    # High - show if > 0, magenta badge
    if [[ ${counts[high]} -gt 0 ]]; then
        hud+="${bmag:-}▲${counts[high]}${reset:-} "
        has_findings=true
    fi

    # Medium - show if > 0, yellow badge
    if [[ ${counts[medium]} -gt 0 ]]; then
        hud+="${byellow:-}●${counts[medium]}${reset:-} "
        has_findings=true
    fi

    # Low - show if > 0, blue badge
    if [[ ${counts[low]} -gt 0 ]]; then
        hud+="${bblue:-}○${counts[low]}${reset:-} "
        has_findings=true
    fi

    # Info - only show if explicitly enabled or has findings and always_show
    if [[ ${counts[info]} -gt 0 ]] && [[ "${HUD_SHOW_INFO:-false}" == "true" ]]; then
        hud+="${blue:-}◇${counts[info]}${reset:-} "
        has_findings=true
    fi

    # If no findings but always_show requested, show zero state
    if [[ "$has_findings" == "false" ]] && [[ "$always_show" == "true" ]]; then
        hud="${g2:-}◆0 ▲0 ●0 ○0${reset:-}"
    fi

    _UI_HUD_CACHE_VAL="${hud% }"
    _UI_HUD_CACHE_TIME="$now"

    if [[ "$has_findings" == "true" ]] || [[ "$always_show" == "true" ]]; then
        printf "%s" "$_UI_HUD_CACHE_VAL"
    fi
}
...
ui_live_progress_update() {
    ...
    local line="$1"
    local hud
    hud=$(ui_get_vulns_summary)

    local spinner="${_UI_SPINNER_CHARS[$_UI_SPINNER_IDX]}"
    _UI_SPINNER_IDX=$(( (_UI_SPINNER_IDX + 1) % ${#_UI_SPINNER_CHARS[@]} ))
    ...
    local output
    if [[ -n "$hud" ]]; then
        output=$(printf "%b%s%b %s %b[%s]%b" "${bblue:-}" "$spinner" "${reset:-}" "$line" "${blue:-}" "$hud" "${reset:-}")
    else
        output=$(printf "%b%s%b %s" "${bblue:-}" "$spinner" "${reset:-}" "$line")
    fi

    line=$(ui_truncate_text "$output" "$width")
    _UI_LIVE_ACTIVE=true
...
ui_progress() {
    ...
    if ui_is_tty; then
        local bar
        bar=$(ui_progress_bar "$pct" 12)
        ui_live_progress_begin
        ui_live_progress_update "▶ [${current}/${total}] ${bar} ${pct}% | ETA: ${eta} | ${step} | ${counters}"
    else
        :
    fi
}
...
ui_batch_start() {
    ...
    if [[ -n "$batch_num" ]] && [[ -n "$batch_total" ]]; then
        batch_label="${batch_num}/${batch_total}: "
    fi

    # Premium mode: use subtle Unicode separators
    if [[ "${_UI_PREMIUM_MODE:-false}" == "true" ]]; then
        printf "  %b┌─ %s%s (%d jobs) ─┐%b\n" \
            "${g2:-}" "$batch_label" "$label" "$total_jobs" "${reset:-}"
    else
        printf "  %b── %s%s [%d jobs] ──%b\n" \
...
ui_batch_end() {
    ...
    # Premium mode: use subtle Unicode separators and cleaner formatting
    if [[ "${_UI_PREMIUM_MODE:-false}" == "true" ]]; then
        # Compact status line with icons
        local status_line=""
        [[ $ok -gt 0 ]] && status_line+="${pgreen:-}✔${ok}${reset:-} "
        [[ $warn -gt 0 ]] && status_line+="${pam:-}⚠${warn}${reset:-} "
        [[ $fail -gt 0 ]] && status_line+="${prose:-}✖${fail}${reset:-} "
        [[ $skip -gt 0 ]] && status_line+="${g2:-}⊘${skip}${reset:-} "
        [[ $cache -gt 0 ]] && status_line+="${cyan:-}♻${cache}${reset:-} "

        printf "  %b└─ %s│ %d/%d │ %ds ─┘%b\n" \
            "${g2:-}" "$status_line" "$completed" "$total" "$elapsed" "${reset:-}"
    else
...
ui_summary() {
    ...
    # Responsive layout: use borderless compact layout for narrow terminals
    if [[ "${_UI_COMPACT_MODE:-false}" == "true" ]] || (( width < 60 )); then
        # Compact borderless layout for narrow terminals
        printf "\n%b▓▓ SCAN COMPLETE: %s%b\n" "${bgreen:-}" "$target" "${reset:-}"
        printf "  Mode: %b%s%b | Duration: %b%s%b\n" "${bblue:-}" "$mode_label" "${reset:-}" "${bblue:-}" "$duration" "${reset:-}"
        printf "  Subs: %b%s%b | Web: %b%s%b\n" "${blue:-}" "$subs" "${reset:-}" "${blue:-}" "$webs" "${reset:-}"
        printf "  Vulns: C:%b%s%b H:%b%s%b M:%b%s%b L:%b%s%b I:%b%s%b\n" \
            "${bred:-}" "$crit" "${reset:-}" \
            "${bmag:-}" "$high" "${reset:-}" \
            "${byellow:-}" "$med" "${reset:-}" \
            "${bblue:-}" "$low" "${reset:-}" \
    ...
    # Full dashboard with box-drawing for wide terminals
    local box_width=$(( width < 80 ? width : 80 ))
    ...
    # Title line
    local title=" SCAN COMPLETE: ${target} "
    local title_len=${#title}
    local padding=$(( (inner_width - title_len) / 2 ))
    printf "%b║%b" "${bgreen:-}" "${reset:-}"
    for ((i=0; i<padding; i++)); do printf " "; done
    printf "%b%s%b" "${bgreen:-}" "$title" "${reset:-}"
    for ((i=padding+title_len; i<inner_width; i++)); do printf " "; done
    printf "%b║%b\n" "${bgreen:-}" "${reset:-}"
    ...
    # Info rows
    printf "%b║%b  Mode:    %b%-20s%b  Duration: %b%s%b" "${bgreen:-}" "${reset:-}" \
        "${bblue:-}" "$mode_label" "${reset:-}" "${bblue:-}" "$duration" "${reset:-}"
    for ((i=38; i<inner_width; i++)); do printf " "; done
    printf "%b║%b\n" "${bgreen:-}" "${reset:-}"

    printf "%b║%b  Output:  %b%s%b" "${bgreen:-}" "${reset:-}" "${bblue:-}" "$outdir" "${reset:-}"
    local outdir_len=${#outdir}
    for ((i=10+outdir_len; i<inner_width; i++)); do printf " "; done
    printf "%b║%b\n" "${bgreen:-}" "${reset:-}"
    ...
    # Findings header
    printf "%b║%b  %bFINDINGS FUNNEL%b" "${bgreen:-}" "${reset:-}" "${bblue:-}" "${reset:-}"
    for ((i=17; i<inner_width; i++)); do printf " "; done
    printf "%b║%b\n" "${bgreen:-}" "${reset:-}"

    # Data rows with colored severity badges
    printf "%b║%b  Source Scope:       %b%6s%b          Web Assets:     %b%6s%b %b▼%b" \
        "${bgreen:-}" "${reset:-}" "${white:-}" "$subs" "${reset:-}" \
        "${blue:-}" "$webs" "${reset:-}" "${blue:-}" "${reset:-}"
    for ((i=62; i<inner_width; i++)); do printf " "; done
    printf "%b║%b\n" "${bgreen:-}" "${reset:-}"

    printf "%b║%b  Critical: %b%6s%b       High: %b%6s%b      Med/Low: %b%6s%b" \
        "${bgreen:-}" "${reset:-}" "${bred:-}" "$crit" "${reset:-}" \
        "${bmag:-}" "$high" "${reset:-}" "${byellow:-}" "$((med + low))" "${reset:-}"
    for ((i=56; i<inner_width; i++)); do printf " "; done
    printf "%b║%b\n" "${bgreen:-}" "${reset:-}"

    printf "%b║%b  Info/Data: %b%6s%b" \
        "${bgreen:-}" "${reset:-}" "${blue:-}" "$info" "${reset:-}"
    for ((i=20; i<inner_width; i++)); do printf " "; done
    printf "%b║%b\n" "${bgreen:-}" "${reset:-}"
...
ui_dryrun_reset() {
    _UI_DRYRUN_COUNT=0
    _UI_DRYRUN_COMMANDS=0
    _UI_DRYRUN_FULL_COMMANDS=()
    local dryroot="${UI_DRYRUN_DIR:-${dir:-/tmp}}"
    _UI_DRYRUN_SHARED_FILE="${dryroot}/.tmp/.dryrun_commands.${PPID:-$$}.${RANDOM}.log"
